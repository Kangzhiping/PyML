...
CELEBA_DATE_DIR = 'C:\BaiduNetdiskDownload\CelebA\Img\img_align_celeba\img_align_celeba'
...
with tf.Session() as sess:
    ...
    # 恢复前一次训练
    ckpt = tf.train.get_checkpoint_state('EBGANmoudle/')
    if ckpt != None:
    ...
    for i in range(40):
        for j in range(num_batch):
            ...
            # 保存模型并生成图像
            if step % 100 == 0:
                saver.save(sess, "EBGANmoudle/celeba.model", global_step=step)
                ...
                for k in range(5):
                    ...
                    misc.imsave('EBGANmoudle/fake_image' + str(step) + str(k) + '.jpg', image)
