...
#嗯，这里说明一下，这里的参数是基于已有模型恢复的参数，意思就是说之前训练过模型，保存过神经网络的参数，现在再取出来作为初始化参数接着训练。
#如果是第一次训练，就用sess.run(tf.global_variables_initializer())，也就不要用到 module_file = tf.train.latest_checkpoint() 和saver.store(sess, module_file)了。
